...
subroutine prolates10_integer(inormal,gamma,n,chi,n1,n2,coefsps)
implicit real*10 (a-h,o-z)
real*10, allocatable, intent(out)  :: coefsps(:)
!
...
real*10, allocatable :: as(:),bs(:),coefsps0(:)
...
call prolates10_trieigen(nn,as,bs,idx,chi,coefsps0)
...
xx =0 
call prolates10_integer_evalder(n,n1,n2,coefsps,xx,valps0,derps0)
call prolates10_legen0(n,val0,der0)
...
subroutine prolates10_integer2(inormal,gamma,n,chi,n1,n2,coefsps,mm)
implicit real*10 (a-h,o-z)
real*10, allocatable, intent(out)  :: coefsps(:)
!
!  Calculate the Sturm-Liouville eigenvalue chi_n(\gamma^2) and construct the 
...
subroutine prolates10_integer_eval(n,n1,n2,coefsps,x,valps)
implicit real*10 (a-h,o-z)
real*10 :: coefsps(n1:n2)
!
...
subroutine prolates10_integer_evalder(n,n1,n2,coefsps,x,valps,derps)
implicit real*10 (a-h,o-z)
real*10 :: coefsps(n1:n2),x,valps,derps
!
...
real*10, allocatable :: pols(:), ders(:)
!real*10, allocatable :: alfs(:)

! Determine how many Legendre polynomials we need to evaluate
m = n+2*n2
allocate(pols(0:m), ders(0:m) )
call prolates10_legeders(m+1,x,pols,ders)
...
subroutine prolates10_integer_lambda(gamma,n,n1,n2,coefsps,zlam)
implicit real*10 (a-h,o-z)
real*10  :: coefsps(n1:n2)
double complex    :: zlam
!
...
x = 0.0d0
call  prolates10_integer_evalder(n,n1,n2,coefsps,x,valps,derps)
...
subroutine prolates10_dimension(gamma,eps,ndim)
implicit real*10 (a-h,o-z)
!
...
do m=m0,10000000
call prolates10_integer(inormal,gamma,m,chi,n1,n2,coefsps)
call prolates10_integer_lambda(gamma,m,n1,n2,coefsps,zlam)
...
subroutine prolates10_leges(n,x,pols)
implicit real*10 (a-h,o-z)
real*10           :: pols(:)
!
...
subroutine prolates10_legeders(n,x,pols,ders)
implicit real*10 (a-h,o-z)
integer                       :: n
real*10              :: x
real*10, intent(out) :: pols(:),ders(:)
!
...
subroutine prolates10_legen0(n,val,der)
implicit real*10 (a-h,o-z)
!
...
subroutine prolates10_lege0(dnu,val,der)
implicit real*10 (a-h,o-z)
!
...
subroutine prolates10_trieigen(n,as,bs,j,chi,y)
implicit real*10 (a-h,o-z)
real*10             :: as(n),bs(n),y(n)
!
...
real*10, allocatable   :: as0(:),bs0(:),x(:)
!
!  Use sturm bisection to estimate  the eigenvalue
!

eps = epsilon(0.0d0)*10
call prolates10_sturm_bisection(eps,n,as,bs,j,chi)
!
...
subroutine prolates10_sturm_bisection(eps,n,as,bs,j,dlambda)
implicit real*10 (a-h,o-z)

integer, intent(in)            :: n,j
real*10, intent(in)   :: eps,as(n),bs(n)
real*10, intent(out)  :: dlambda
!
...
real*10, allocatable :: pols(:),pols0(:)
